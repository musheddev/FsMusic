module NLfloatynamics


let rungeKutta (fn :float->float) (x :float) (dt :float) = //fn x -> dx/dt
    let k1 = fn(x)*dt
    let k2 = fn(x + 0.5*k1)*dt
    let k3 = fn(x + 0.5*k2)*dt
    let k4 = fn(x + k3)*dt
    x + (1./6.)*(k1 + 2.*k2 + 2.*k3 + k4)

let secondOrder_RK (fn1 :float*float->float) (fn2 :float*float->float) (x1 :float) (x2 :float) (dt :float) =
    let k1 = fn1(x1,x2)*dt
    let l1 = fn2(x1,x2)*dt
    ...
    let xdot1 = x1 + (1./6.)*(k1 + 2.*k2 + 2.*k3 + k4)
    let xdot2 = x2 + (1./6.)*(l1 + 2.*l2 + 2.*l3 + l4)
    xdot1,xdot2

let nOrder_RK (fns :(float [] -> float) []) (xs :float []) (dt :float) =
    ...
    Array.map (fun ((x,k1,k2),(k3,k4)) -> x + (1./6.)*(k1 + 2.*k2 + 2.*k3 + k4)) primes

let wave (amp :float) (w :float) (phase :float) (t :float) =
    amp * sin(w*t + phase)

let modu (x :float) (modu :float) =
    let z = x/modu
    (z - (Math.Round z))*modu


let ramp (amp :float) (w :float) (phase :float) (t :float) =
    (amp*(w*(modu (t + phase) 6.28))/6.28)

//mode locking equations from Fletcher 1999

let adot (g :(float->float)->float) (a :float) (amp :float) (w :float) (phase :float) (yn :float->float->float->float->float) (t :float) =
    g(yn amp w phase) * cos(w*t + phase) / w - a*amp/2.

let fdot (g :(float->float)->float) (amp :float) (w :float) (phase :float) (yn :float->float->float->float->float) =
    - g(yn amp w phase) * sin(w + phase) / (w*amp)

let makeg (c1 :float) (c2 :float) (p :float) (q :float) = //linear case but summation doesn't make sense  
    fun (yfn :float->float) ->
         let y fn = List.sumBy (fun x -> fn(float x)*0.01) [0.0 .. 0.01 .. 6.28]
...
let make_fdots_2 modei modej c1 c2 =
    ...
    let fj xj t deltai thetai deltaj thetaj ampi ampj =
        (modej.floatriver/(modej.Freq*xj))
    ...
    let fi xi t deltai thetai deltaj thetaj ampi ampj =
        (modei.floatriver/(modei.Freq*xi))
...
let test1() =
    let fn (x :float) = x*(1.-x)
    let dt = 0.1
    let chart = 
        List.mapFold (fun (x :float) t -> 
            let xn = rungeKutta fn x (float dt)
            ((t, float xn),xn)) 1.5 [for t in 0. .. dt .. 10. -> t]
    printfn "%O" (fst chart)        
...
let alpha = float 0.2
let dt = 0.01
let freq = float 120.0
let amp = float 1.0

let make_chart c1 c2 freq amp alpha dt (detuple :float*float -> float) =
    let g = makeg c1 c2 0.0 0.0
    let f = (fun (x1,x2) -> fdot g x1 x2 0. wave)
    let a = (fun t (x1,x2) -> adot g alpha x1 x2 0. wave t)
    let chart = 
        List.mapFold (fun ((x1,x2) :float*float) t -> 
            let y = secondOrder_RK f (a (float t)) x1 x2 (float dt)
            ((t,detuple y),y)) (freq,amp) [for t in 0. ..dt ..10. -> t]
    chart |> fst

let charts1 = 
    List.map (fun x -> make_chart c1 c2 freq amp (float x) dt (fst >> float) |> Chart.Line) [0.0 .. 0.1 .. 1.]

let charts2 = 
    List.map (fun x -> make_chart c1 c2 freq (float x) alpha dt (snd >> float) |> Chart.Line) [0.0 .. 0.1 .. 1.]
        
let chart1 = (make_chart 0.1 0.01 120. 0.5 0.1 dt (snd >> float) |> Chart.Line).ShowChart()
let chart2 = (make_chart 0.1 0.01 120. 0.5 0.1 dt (fst >> float) |> Chart.Line).WithYAxis(Max = 121., Min = 119.).ShowChart()
